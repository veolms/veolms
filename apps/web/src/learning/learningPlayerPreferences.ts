...
export const getLearningPlayerBootstrapScript = () =>
  `(()=>{const r=document.documentElement;let a=true,u=false,p=1,v=1;try{const as=localStorage.getItem(${JSON.stringify(lessonPlayerStorageKeys.autoplay)});if(as!==null)a=as==="on"||as==="true";const ms=localStorage.getItem(${JSON.stringify(lessonPlayerStorageKeys.muted)});u=ms==="true"||ms==="on";const pr=Number(localStorage.getItem(${JSON.stringify(lessonPlayerStorageKeys.playbackRate)}));if(Number.isFinite(pr)&&pr>0)p=pr;const vs=localStorage.getItem(${JSON.stringify(lessonPlayerStorageKeys.volume)});if(vs!==null&&vs.trim()){const n=Number(vs);if(Number.isFinite(n))v=Math.min(1,Math.max(0,n))}}catch{}const s={autoplay:a,muted:u,playbackRate:p,volume:v};window.__VEO_BOOTSTRAP__={...window.__VEO_BOOTSTRAP__,player:s};r.dataset.playerAutoplay=a?"on":"off";r.dataset.playerMuted=u?"true":"false";r.dataset.playerPlaybackRate=String(p);r.dataset.playerVolume=String(v)})();`;
