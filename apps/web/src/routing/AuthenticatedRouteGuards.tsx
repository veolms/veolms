...
function shouldBlockAcademyRender(
  ...
  if (isGuestLandingPath(path)) return true;
  if (!requiresAcademyAuth(path)) return false;
  if (!access.isAuthenticated) return true;
  return access.needsMfaChallenge;
}

export function AuthenticatedAcademyRouteGuard({
  children,
}: {
  children: ReactNode;
}) {
  ...
  useEffect(() => {
    ...
    if (!authenticationRequired || pending) return;
  ...
  if (
    (authenticationRequired && pending) ||
...
export function AuthRouteGuardRuntime() {
  const location = useLocation();
  const navigate = useNavigate();
  ...
  useEffect(() => {
    if (pending || path === "/auth/callback") return;
    ...
    if (path === "/mfa-setup") {
      ...
      if (access.isSessionReady) navigate(APP_HOME_PATH, { replace: true });
      return;
    }

    if (access.needsMfaChallenge) {
      if (path !== "/login") navigate(MFA_CHALLENGE_PATH, { replace: true });
      return;
    }

    if (!access.isSessionReady) return;
  ...
  if (pending) return <AppLoadingScreen variant="embedded" />;
  if (path === "/auth/callback") return <Outlet />;
